...
1. 一般的轨迹都是以正弦波的形式来生成的，即为simulink里的`Sine Wave`模块生成，只需要`Sine Wave`正弦波模块、`Outpart`输出模块、`Mux`模块与`To Workspace`模块，较为简单。

如下图展示：

![轨迹生成 （1）](/imgs/2025-04-29/pFWBZW8TqOlMpRLl.png "轨迹生成 （1）")

2. 也可以通过`Fcn`模块自己写函数式来定义轨迹的形状，较为推荐此方式来进行轨迹的生成。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyODM3OTcyMywzNDI0NzY5NTddfQ==
-->